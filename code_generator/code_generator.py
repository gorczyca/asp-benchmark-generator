...
from project_info import GITHUB_PAGE, PROJECT_VERSION, AUTHOR_EMAIL
...
def generate_code(model: Model, show_all_predicates: bool, shown_predicates_dict: Dict[str, bool]):
...
def __generate_variable_number_of_port_instances(name: str, cmp: Component, count: int, prt_number: int, offset: int) -> str:
    instance_code = f'{name}{DOMAIN_STRING}({offset+1}..{offset+count}).\n'     # This rule is added for solving purposes
        ...
        if count:   # If component appears in configuration
            ...
            for prt_id, prt_count in cmp.ports.items():
                ...
                for prt_individual_name in prt_individual_names:
                    ...
                    offset += count
                    if cmp.symmetry_breaking:
                        prt_instance_code += __generate_symmetry_breaking_rule(prt_individual_name, variable=PRT_VARIABLE)
                inst_code += prt_instance_code
        inst_code += '\n'
    return inst_code, inst_predicates


def __generate_show_directives(show_all_predicates: bool, shown_predicates_dict: Dict[str, bool], instances_predicates: List[str]) \
        -> str:
